...
const { runInExcutable, absAppRunningPath, absAppStaticsPath, absAssetsPath } = reaxel_ElectronENV();
...
function spawnWar3AHK({ ahkSpawner_SetState, ahkSpawner_Store }) {
	...
	const ahk_bin_path = path.join(absAssetsPath, 'ahk-bin/v1/AutoHotkey.exe');
	const ahkScriptPath = path.join(absAppStaticsPath, "assets/ahk-scripts/war3.ahk");
	const ahk = cp.spawn(ahk_bin_path, [ahkScriptPath], {
